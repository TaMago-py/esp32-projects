...
// "true" for a faced cube, 'false' for just the edges (Inputs)
bool solid = false;
 ...
 * and projects the results onto the 2D space,so it can be shown on the screen.
*/
void updateCube() {
  for (int i = 0; i < 8; i++) {
...
/**
 * @brief Draws the cube on the screen using the already processed 
 * vertices given by the updateCube() function.
 * 
 * @param display The display to show the Cube
 */
...
/**
 * @brief Checks if a face is visible by evaluating the cross point of two vectors. 
 * These vectors are formed by three consecutive vertices from one of the cube's faces.
 * 
 * @param vertex_0 Face vertex
 * @param vertex_1 Face next vertex from vertex_0
 * @param vertex_2 Face next vertex from vertex_1
 * 
 * @return cross_product >= 0, being this variable the cross product
 * between the vectors formed from vertex_0 to vertex_1 and
 * from vertex_0 to vertex_2
 */
bool isFaceVisible(int vertex_0, int vertex_1, int vertex_2) {
  ...
  long cross_product = x1 * y2 - x2 * y1;

  return cross_product >= 0;
}

/**
 * Draws the cube's faces if they are visible.
 * This function only executes if the solid mode is activated.
 * 
 * @param p0 Face vertex
 * @param p1 Face next vertex from p0
 * @param p2 Face next vertex from p1
 * @param p3 face next vertex from p2
 */
